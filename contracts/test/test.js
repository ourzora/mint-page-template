...
describe('YourContractMinimal', function () {
  let contract
  let splitter
  let mintPrice, maxSupply, maxMintCount, artistProofCount, _baseURI
  let owner, bob, alice
  let managerRole, adminRole

  beforeEach(async () => {
    ;[owner, bob, alice] = await ethers.getSigners()
    const Contract = await ethers.getContractFactory('YourContractMinimal')
    contract = await Contract.deploy()
    await contract.deployed()
    managerRole = await contract.MANAGER_ROLE()
    adminRole = await contract.DEFAULT_ADMIN_ROLE()
  ...
  it('Ownership can be transferred', async function () {
    ...
    await expect(contract.connect(owner).setSaleActive(true)).to.be.revertedWith(
      `AccessControl: account ${owner.address.toLowerCase()} is missing role ${managerRole}`
    )
  })
  ...
  it('Sale can be paused', async function () {
    // Unpause contract
    await contract.connect(owner).setSaleActive(true)
